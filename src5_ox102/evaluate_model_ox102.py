#!/usr/bin/env python3
"""
evaluate_model_ox102.py — Version Oxford 102 (src5_oxford102)
...
Adaptations Oxford 102 : split officiel train/val/test, matrice de confusion
102×102 sans annotations, ROC micro-average, Top-1/3/5.

Run from src5_oxford102/ with: python evaluate_model_ox102.py --seed 42
"""
...
# Project imports
from load_data_ox102 import load_flower_data, OXFORD_CLASS_NAMES
from model_ox102 import create_model, FlowerResNet 
from train_ox102 import prepare_dataloaders, get_transforms, FlowerDataset

import argparse
warnings.filterwarnings('ignore')

eval_parser = argparse.ArgumentParser(description='Évaluation Groupe C')
...
CHECKPOINT_PATH = SRC_DIR / 'checkpoints' / f'run_seed{eval_args.seed}' / 'best_model_ox102.pth'
...
class ModelEvaluator:
    ...
    def __init__(self):
        self.model: Optional[FlowerResNet ] = None
    ...
    def load_data_and_model(self):
        ...
        # --- Split officiel Oxford 102 (identique à l'entraînement, par construction) ---
        # Les images sont concaténées dans l'ordre [train | val | test] par le loader.
        n_train, n_val, n_test = split_sizes

        self.test_images = images[n_train + n_val:]
        self.test_labels = labels[n_train + n_val:]

        # Also keep train+val for K-Fold
        self.train_val_images = images[:n_train + n_val]
        self.train_val_labels = labels[:n_train + n_val]
    ...
    def evaluate_confusion_matrix(self):
        ...
        # Save as CSV
        csv_path = RESULTS_DIR / 'confusion_matrix_B.csv'
        ...
        ax.set_title('Confusion Matrix — Oxford 102', fontsize=14, fontweight='bold')
        plt.colorbar(im, ax=ax)

        tick_marks = np.arange(0, len(CLASS_NAMES), 10)
        ax.set_xticks(tick_marks)
        ax.set_yticks(tick_marks)
        ax.set_xticklabels(tick_marks, rotation=90, fontsize=7)
        ...
        plt.tight_layout()
        raw_path = RESULTS_DIR / 'confusion_matrix_ox102.png'
        plt.savefig(raw_path, dpi=150, bbox_inches='tight')
        plt.close()
        print(f"   Raw plot saved: {raw_path}")
        
        # --- Normalized confusion matrix plot (même adaptation 102 classes) ---
        fig, ax = plt.subplots(figsize=(18, 16))
        im = ax.imshow(cm_normalized, cmap='Blues', vmin=0, vmax=1)
        ax.set_title('Normalized Confusion Matrix — Oxford 102', fontsize=14, fontweight='bold')
        plt.colorbar(im, ax=ax)
        ...
        plt.tight_layout()
        norm_path = RESULTS_DIR / 'confusion_matrix_normalized_ox102.png'
    ...
    def evaluate_roc_auc(self):
        ...
        # Plot — avec 102 classes, tracer une courbe par classe est illisible.
        # Usage standard : courbe micro-average (toutes classes confondues) ;
        # les AUC par classe restent disponibles dans le rapport JSON.
        fpr_micro, tpr_micro, _ = roc_curve(labels_onehot.ravel(), self.all_probs.ravel())
        micro_roc_auc = roc_auc_score(labels_onehot, self.all_probs, average='micro', multi_class='ovr')

        fig, ax = plt.subplots(figsize=(10, 8))
        ax.plot(fpr_micro, tpr_micro, color='steelblue', lw=2,
                label=f"Micro-average (AUC = {micro_roc_auc:.3f})")
        ax.plot([0, 1], [0, 1], 'k--', lw=1, label='Random Classifier')
        ax.set_xlim([0.0, 1.0])
        ax.set_ylim([0.0, 1.05])
        ax.set_xlabel('False Positive Rate', fontsize=12)
        ax.set_ylabel('True Positive Rate', fontsize=12)
        ax.set_title(f'ROC Curve — Micro-average OvR ({NUM_CLASSES} classes)', fontsize=14, fontweight='bold')
        ax.legend(loc='lower right', fontsize=10)
        ax.grid(True, alpha=0.3)

        plt.tight_layout()
        roc_path = RESULTS_DIR / 'roc_curves_ox102.png'
        ...
        self.results['roc_auc'] = {
            ...
            'micro_average': float(micro_roc_auc)
        }
    ...
    def evaluate_calibration(self, n_bins: int = 10):
        ...
        plt.tight_layout()
        calib_path = RESULTS_DIR / 'reliability_diagram_ox102.png'
    ...
    def evaluate_top_k_accuracy(self):
        """Compute Top-1, Top-3 and Top-5 accuracy (usage standard pour 102 classes)."""
        print("\n[*] Computing Top-K accuracy...")
        
        # Avec 102 classes, l'usage standard est Top-1 / Top-5 (comme ImageNet) ; Top-3 en bonus
        top3_acc = top_k_accuracy_score(self.all_labels, self.all_probs, k=3, labels=range(NUM_CLASSES))
        top5_acc = top_k_accuracy_score(self.all_labels, self.all_probs, k=5, labels=range(NUM_CLASSES))
    ...
    """def evaluate_kfold_cross_validation(self):
        ...
        plt.tight_layout()
        kfold_path = RESULTS_DIR / 'kfold_boxplot_C.png'
    ...
    def evaluate_temporal_performance(self):
        ...
        plt.tight_layout()
        throughput_path = RESULTS_DIR / 'inference_throughput_ox102.png'
    ...
    def evaluate_confidence_distribution(self):
        ...
        plt.tight_layout()
        conf_path = RESULTS_DIR / 'confidence_distribution_ox102.png'
    ...
    def evaluate_per_class_errors(self):
        ...
        print("   Top 10 des classes avec le plus d'erreurs:")
        sorted_errors = sorted(per_class_error.items(), key=lambda kv: kv[1]['error_rate'], reverse=True)
        for name, stats in sorted_errors[:10]:
            print(f"      {name:30s}: {stats['error_rate']*100:.1f}% error ({stats['correct']}/{stats['total_samples']})")
    ...
    def generate_reports(self):
        ...
        # --- JSON Report ---
        json_path = RESULTS_DIR / 'evaluation_report_ox102.json'
        ...
        summary_lines.append(f"Model: FlowerResNet (Groupe C - Fine-tuning)")
        ...
        summary_lines.append(f"   Top-5: {tk.get('top5', 0)*100:.2f}%")
        summary_lines.append("")
        ...
        for name, stats in pce.get('per_class', {}).items():
            summary_lines.append(f"{name:30s}: {stats['error_rate']*100:5.1f}% error")
        summary_lines.append("")
        ...
        summary_text = "\n".join(summary_lines)
        txt_path = RESULTS_DIR / 'evaluation_summary_ox102.txt'
