...
BASE_DIR = Path(__file__).resolve().parent.parent  # remonte de src1 vers code
...
def main():
    ...
    loader.plot_class_distribution(labels, save_path=CHECKPOINTS_DIR/"5_classes_distrib.png")
    ...
    model.load_state_dict(torch.load(CHECKPOINTS_DIR/"best_model.pth", weights_only=True)['model_state_dict'])    # Courbes d'entraînement
    trainer.plot_history(save_path=CHECKPOINTS_DIR/"train_curves.png")

    # Évaluation sur le test set (charge le meilleur modèle)
    print("\n" + "=" * 70)
    print("  ÉVALUATION SUR LE TEST SET")
    print("=" * 70)
    test_acc = trainer.evaluate(test_loader, class_names, save_path=CHECKPOINTS_DIR/"conf_matrix.png")
    ...
    print(f"  Modèle sauvegardé      : checkpoints/run_seed{SEED}/best_model.pth")
    print(f"  Courbes sauvegardées   : checkpoints/run_seed{SEED}/training_curves.png")
    print("=" * 70)
