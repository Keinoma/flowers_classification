...
class Trainer:
    ...
    def evaluate(self, test_loader: DataLoader, class_names: List[str]):
        """
...
def get_transforms(train: bool = True, image_size: int = 224):
    """
    Retourne les transformations pour train ou validation/test.
    
    Args:
        train: Si True, applique l'augmentation de données
        image_size: Taille des images
        
    Returns:
        Compose de transformations
    """
    if train:
        return transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]) # pr les 3 channels RGB de chaque pxl, va faire: (pxl[rgb]-moyenne)/std
        ])
    else:
        return transforms.Compose([
            transforms.ToTensor(),
...
def prepare_dataloaders(images: np.ndarray, 
    ...
    # bougent pas.
    #
    # - Train : on applique les transformations d'augmentation (flip,
    #           rotation, color jitter) pour enrichir artificiellement
    #           le dataset et éviter le surapprentissage.
