...
from load_data_B import load_flower_data, LoadData
from model_B import create_model
from train_B import Trainer, prepare_dataloaders
...
# ═══════════════════════════════════════════════════════════════
import argparse
parser = argparse.ArgumentParser(description='Entraînement Groupe B — Transfer Learning')
...
def main():
    ...
    loader.plot_class_distribution(labels, save_path=CHECKPOINTS_DIR/"5_classes_distrib_B.png")
    ...
    model.load_state_dict(torch.load(CHECKPOINTS_DIR/"best_model_B.pth", weights_only=True)['model_state_dict'])    # Courbes d'entraînement
    trainer.plot_history(save_path=CHECKPOINTS_DIR/"train_curves_B.png")

    # Évaluation sur le test set (charge le meilleur modèle)
    print("\n" + "=" * 70)
    print("  ÉVALUATION SUR LE TEST SET")
    print("=" * 70)
    test_acc = trainer.evaluate(test_loader, class_names, save_path=CHECKPOINTS_DIR/"conf_matrix_B.png")
    ...
    print(f"  Modèle sauvegardé      : {CHECKPOINTS_DIR}/best_model_B.pth")
    print(f"  Courbes sauvegardées   : {CHECKPOINTS_DIR}/train_curves_B.png")
    print("=" * 70)
